...
def generate_response(key, prompt_query, choices=10, engine="gpt-3.5-turbo", tokens=500):
    ...
    prompt = f'Act has a highly sophisticated natural language processing program. I am going to give you the ' \
             ...
             f'excluded to make grammatical sense. Or, a word may have multiple letters that correspond to the acronym.A' \
             f'Display {str(choices)} relevant acronyms using the following abstract and display them in list format.' \
             f' The abstract is as follows "{prompt_query}" On top of the abstract here are synonyms of keywords ' \
             f'within the abstract that can also be used to supplement acronym generation: "{synonyms}"'
